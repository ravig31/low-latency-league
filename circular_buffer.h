/*
CREDIT: https://github.com/vinitjames/circularbuffer/tree/master
*/
...
//AUTHOR: https://github.com/ravig31
template<typename T>
inline 
typename CircularBuffer<T>::iterator CircularBuffer<T>::erase(const_iterator position) {
	std::lock_guard<std::mutex> _lck(_mtx);
    if (empty() || position._ptrToBuffer != this || position._index >= _size) {
        throw std::out_of_range("Invalid iterator in erase");
    }
    size_type pos = position._index;

    if (pos == 0) {
        _decrement_bufferstate();
        iterator result;
        result._ptrToBuffer = this;
        result._offset = _tail;
        result._index = 0;
        result._reverse = position._reverse;
        return result;
    }
    
	if (pos == _size - 1) {
        --_size;
        _head = (_head == 0) ? _max_size - 1 : _head - 1;
        
        // Return iterator to the end (past the last element)
        iterator result;
        result._ptrToBuffer = this;
        result._offset = _tail;
        result._index = _size;
    ...
    // For elements in the middle, determine which shift is more efficient
    if (pos < _size - pos - 1) {
        // Shift elements from the front towards the position
        for (size_type i = pos; i > 0; --i) {
            size_type current_idx = (_tail + i) % _max_size;
            size_type prev_idx = (_tail + i - 1) % _max_size;
            _buff[current_idx] = std::move(_buff[prev_idx]);
        }
        
        _tail = (_tail + 1) % _max_size;
    } else {
        // Shift elements from the back towards the position
        for (size_type i = pos; i < _size - 1; ++i) {
            size_type current_idx = (_tail + i) % _max_size;
            size_type next_idx = (_tail + i + 1) % _max_size;
            _buff[current_idx] = std::move(_buff[next_idx]);
        }
    ...
    result._ptrToBuffer = this;
    result._offset = _tail;
    result._index = pos;  // Index stays the same but points to the next element now
    result._reverse = position._reverse;
    return result;
}
