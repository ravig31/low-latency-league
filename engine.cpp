...
#define ERROR_LOG(msg) std::cerr << "[ERROR] " << __FILE__ << ":" << __LINE__ << " - " << msg << std::endl

// Overload operator<< for Side
inline std::ostream& operator<<(std::ostream& os, const Side& side) {
    switch (side) {
        case Side::BUY: os << "BUY"; break;
        case Side::SELL: os << "SELL"; break;
...
bool is_valid_index(const std::array<T, N>& array, size_t index) {
    return index < N;
}
...
inline PriceType get_price_index(PriceType price, PriceType currentBase)
{
    ...
    if (index >= BUFFER_SIZE) {
        ERROR_LOG("Price index out of range: " << index << " for price " << price << " and base " << currentBase);
...
)
{
    try {
        ...
        else [[unlikely]]
        {
            ...
            ERROR_LOG("Price level overflow at index " << index);
            // Move to outliers instead of throwing
            outliers[order.price].orders[outliers[order.price].count++] = order.id;
            outliers[order.price].volume += order.quantity;
        }
    } catch (const std::exception& e) {
...
)
{
    try {
        ...
        for (uint16_t i = 0; i < level.count && incoming.quantity > 0;)
        {
            ...
            //     ERROR_LOG("Invalid order ID in level: " << orderId);
            //     // Skip this order
            //     i++;
            //     continue;
            // }
...
)
{
    try {
        ...
        {
            int16_t position = level->find_order(order_id);
            if (position > 0){
                level->remove_order(position);
            }
