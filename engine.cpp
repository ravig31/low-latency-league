...
inline __attribute__((always_inline, hot)) uint32_t process_orders(
	LevelPriceType compPrice, Order& order, PriceLevels& mPriceLevels, OrderLevels& mOrderLevels,
	Volumes& volumes, OrderStore& orders, OrderBitSet& ordersActive, PriceLevels& sPriceLevels,
...
uint32_t match_order(Orderbook& orderbook, const Order& incoming) noexcept
{
	...
	matchCount = process_orders(isSell ? -modifiedPrice : modifiedPrice, order,
